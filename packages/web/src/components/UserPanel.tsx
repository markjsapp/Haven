...
export default function UserPanel() {
  ...
  const voiceChannelName = voiceChannel
    ? parseChannelDisplay(voiceChannel.encrypted_meta, user.id)?.name ?? "Voice"
    : "Voice";
  ...
  return (
    <div className="user-panel">
      ...
      <div className="user-panel-row">
        <div className="user-panel-avatar-wrap" ref={avatarWrapRef} onClick={() => setShowStatusSelector((v) => !v)} role="button" tabIndex={0} aria-label="Change status" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setShowStatusSelector((v) => !v); } }}>
          <Avatar
        ...
        <div className="user-panel-info">
          ...
          {user.custom_status ? (
            <span className="user-panel-custom-status" onClick={() => setShowCustomStatus(true)} title="Edit custom status" role="button" tabIndex={0} aria-label="Set a custom status" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setShowCustomStatus(true); } }}>
              {user.custom_status_emoji && <span>{user.custom_status_emoji} </span>}
              {user.custom_status}
            </span>
          ) : (
            <span className="user-panel-tag" onClick={() => setShowCustomStatus(true)} title="Set a custom status" role="button" tabIndex={0} aria-label="Set a custom status" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setShowCustomStatus(true); } }}>
              {statusConfig.label}
            </span>
          )}
        </div>
